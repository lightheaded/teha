...
- Three documented gaps in Todoist filters that we close from day one: query comment text, show completed tasks, and show a parent task together with its sub-tasks in one result ([introduction to filters](https://www.todoist.com/help/articles/introduction-to-filters-V98wIH)). The terms are `comment:`, `done` with `completed`, `wont do` and `any state`, and `with subtasks:`. Each one has a test on the grammar and a test against real rows. `comment:` reads the comment table since 2026-09-02. It reached the description before that, which was right while a comment lived there: see D-020.
...
- Full-text search over titles, descriptions, comments.
  - **Titles and descriptions since 2026-08-25.** The fts5 index is written on a task change, so it holds no comment text. `comment:` reads the comment table instead, which is a LIKE and not an index: see [BACKLOG.md](BACKLOG.md).
...
- Household space: projects shared with one other account, assignee per task, "assigned to me" filter, comments with attachments (images, files, up to a size cap), reactions.
  - **All of it runs except the attachments, since 2026-09-02.** An invitation, a shared project, an assignee, the `assigned to:` term and comments all work, in the server, the browser, the phone and the filter grammar. A comment is a row with an author, and only the author changes it: see [DECISIONS.md](DECISIONS.md) D-016 and D-020. An attachment needs a file store, which no part of this build has. A reaction is not built.
- Push notifications: UnifiedPush through ntfy for Android, Web Push for browsers and iOS PWA. Reminder at time, before due, daily digest, comment and assignment notifications.
  - **Web Push runs since 2026-08-27**, with the three reminder kinds: at the due time, before the due time and a daily digest. `internal/push` holds the scheduler and the sender. **A comment and an assignment notify since 2026-09-02:** the store reports the fact and the sender writes the words, so a second transport needs no second copy. See D-020. **The phone notifies since 2026-09-03**, and it is not push: `SyncWorker` runs a background sync, and a nudge is what a pull brought that the database did not hold. The repository reports the fact and `Notifier` writes the words, which is the same split. So the latency is the sync interval, about a quarter of an hour. UnifiedPush is still the answer for a phone that has to hear at once, and [BACKLOG.md](BACKLOG.md) records when it becomes worth building.
- Shopping mode per project: items grouped by category (learned from history, editable), big check targets, recently bought suggestions, quantity in the title (`2x milk`), live sync while two people are in the store, checked items collapse and clear on request.
  ...
  - **It runs in the browser since 2026-09-02, and on the phone since 2026-09-03.** An aisle is a section of the project and the suggestions are the completed tasks of it, so nothing here is a new table. The aisle of a new item is copied from the newest item of the same name. `2x milk` draws the count as a chip. The basket holds what went in on this trip, and a button empties it. The screenshot job fails the build if the layout scrolls sideways at 320 pixels or if a check target shrinks below a thumb. See D-021. The phone draws the same layout, with a 64 dp row that ticks anywhere.
...
- Widgets on Android (Glance): Today list with a filter, add button.
  - **Not built.** The Quick Settings tile carries the capture half of it. A widget is a second surface with its own layout and its own update path, and nothing in daily use has asked for it yet.
...
| Web | Svelte 5.56, SvelteKit 2.70, Vite, TypeScript, IndexedDB (one object store per table), virtual lists | Small bundle, fast, compiles away. The storage engine is IndexedDB and not wa-sqlite in OPFS: the browser has no use for a query engine, because the renderer and the filter evaluator both work on rows in memory. See D-022. |
...
One grammar and one fixture corpus. The Go parser compiles a query to a SQL `WHERE` clause with parameters, and it takes the table and column names as a value, so one parser emits the names of the server database and the names of a client store. The Android app therefore calls the same compiler through the gomobile binding (D-002, D-014) and needs no parser of its own. The browser reads the whole grammar in JavaScript over its local rows, because it holds objects and not SQL, and one shared corpus of queries and answers holds the two implementations together (D-018). A term that needs a column a store has not got fails there with one sentence, and never answers with the wrong rows. The grammar is Todoist's, plus the Phase 3 extensions. `comment:` reads the comment table since 2026-09-02 (D-020).
...
| M1 Core server | Schema, sync, filters, recurrence, export, import | Todoist account imports with zero loss. Property test converges. | **Done, 2026-08-27.** One property per sync promise of §6.1, over a seeded command stream in two legal orders. Zero loss proven against a synthetic fixture that holds only the hard cases, with the resume cut at every command. Four defects found and fixed: bugs 7 to 10 in [POC.md](POC.md). Accounts: passkeys ship for the browser beside the device token, per [DECISIONS.md](DECISIONS.md) D-009. The password fallback is not built, because the token already is one. Open items, none of them blocking, are in [BACKLOG.md](BACKLOG.md): no client writes an order key yet, and a comment and a saved filter still have no table. |
| M2 Web | Views, quick add, keyboard, offline, PWA | Author uses the web app for one week without Todoist | Views, quick add, the task detail, the keyboard, offline and the service worker run. A project view draws its sections as a board or as a shopping list, every view draws as a month or a week calendar with drag to reschedule, a note reads as Markdown, a task carries a conversation, and a filter field takes any query the grammar knows. The local copy is in IndexedDB, so the last known shortcut in the web app is closed: see D-022. The week has not started. |
| M3 MCP | Tools, token auth, Claude Code config | An agent plans the day in three calls, never times out | Ten tools, stateless transport, token auth. `plan_day` answers in one call and 114 tokens. An agent reads the comments on a task and leaves one. |
| M4 Android | Offline core, tile, share, gestures, Obtainium | Author uninstalls Todoist from the phone | **Everything it names is built, 2026-09-03.** The app ships through Obtainium. Offline core, the tile, the detail screen, bulk actions, the six views of the browser, a view per project, a filter field, the household, an assignee, a section, comments, shopping mode, the share target and a background sync all run. A notification arrives with the sync and not with a push, which [BACKLOG.md](BACKLOG.md) explains. The exit test is the author uninstalling Todoist, and it has not started. |
| M5 macOS | Tauri app, global shortcut, URL scheme | Author removes the Todoist hotkey | Built. The shell hosts the web app, the shortcut opens a quick add panel, the menu bar icon and `teha://add?text=` work, and the token is in the keychain. Unsigned, per section 4. The shell has never been compiled: no Rust toolchain was available, so CI on macOS is what proves it. The exit test needs a week of use. |
| M6 Household | Sharing, comments, push, shopping mode | Partner uses it for groceries for one month without asking for Todoist. Two phones tick items in the same shop and neither buys a duplicate | **Everything it names is built, 2026-09-02.** An invitation makes an account with its own inbox and its own device token, a list is shared from the sidebar, a task carries an assignee, comments are rows with authors, an assignment and a comment notify, shopping mode runs in the browser, and every read and write is scoped to the account that asked: see [DECISIONS.md](DECISIONS.md) D-016, D-017, D-020 and D-021. The phone joins the household, assigns a task and files one under a heading. Attachments are open, because no part of this build stores a file. The exit test needs a month of shopping and has not started. |
...
| M8 Open | F-Droid, docs, hosted pilot | A stranger self-hosts from the README in under 15 minutes | Dockerfile, compose, kustomize and a deployment guide exist. The restore is rehearsed by `scripts/restore-drill.sh`, which found that the backup was replicating a stale file: see D-019. |
...
(D-003), the iOS answer (D-004), the once-only reminder (D-010), the
missed-reminder window (D-011) and the naming scheme in the filter compiler
(D-014).

Some choices in this plan changed when the code met reality. Each one is small, and each one has a reason.
...
| The web client holds SQLite in OPFS | The web client holds its rows in IndexedDB, one object store per table | The shortcut that had to go was the single `localStorage` string: five megabytes of quota, rewritten whole on every keystroke. IndexedDB removes both, with no new dependency. SQLite in the browser buys a query engine, and the rows are in memory anyway for the renderer and the row evaluator. See D-022. |
...
| One SQLite file holds one account | One file holds one household | Two accounts that share a shopping list in two files need a second sync protocol between the files. One file needs none: a shared row is one row. One file per tenant returns with the hosted build. See D-016. |
| The browser gets the shared filter compiler | The browser has a second evaluator, held by a shared corpus | The browser holds objects and not SQL, so the compiler alone answers nothing there. A row evaluator had to be written either way, and WebAssembly of it costs 900 kB in an app that must start with no network. See D-018. |
| A schema change on the phone costs one full pull | A schema change on the phone is a Room migration | The cache can be thrown away and the outbox cannot. Dropping the outbox loses a capture that never reached the server, which is the one promise the app makes about writing offline. The household migration adds the columns and drops the watermark. |
| Shopping mode groups by a category of its own | Shopping mode groups by the section of the project | A section is already a heading a person can rename, reorder and drag, in every client. A category table beside it is a second heading with the same job. See D-021. |
...
- Licence: Apache-2.0 for the shared packages and the clients, AGPL-3.0-or-later for the server. See [LICENSING.md](../LICENSING.md) and D-001. A client that talks over HTTP only stays a separate program, so section 13 of the AGPL reaches the server alone. A self-hoster who modifies the server must offer its source to the people who use it.
