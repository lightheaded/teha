...
- Shopping mode per project: items grouped by category (learned from history, editable), big check targets, recently bought suggestions, quantity in the title (`2x milk`), live sync while two people are in the store, checked items collapse and clear on request.
  ...
  - **It runs in the browser since 2026-09-02.** An aisle is a section of the project and the suggestions are the completed tasks of it, so nothing here is a new table. The aisle of a new item is copied from the newest item of the same name. `2x milk` draws the count as a chip. The basket holds what went in on this trip, and a button empties it. The screenshot job fails the build if the layout scrolls sideways at 320 pixels or if a check target shrinks below a thumb. See D-021. The phone has the list and not the layout.
