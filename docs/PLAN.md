...
| M1 Core server | Schema, sync, filters, recurrence, export, import | Todoist account imports with zero loss. Property test converges. | **Done, 2026-08-27.** One property per sync promise of §6.1, over a seeded command stream in two legal orders. Zero loss proven against a synthetic fixture that holds only the hard cases, with the resume cut at every command. Four defects found and fixed: bugs 7 to 10 in [POC.md](POC.md). Open items, none of them blocking, are in [BACKLOG.md](BACKLOG.md): no client writes an order key yet, and a section, a comment and a saved filter still have no table. |
